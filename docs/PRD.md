...
| | |
|---|---|
| **Status** | DRAFT — v0.7 (see document history) |
| **Date** | 2026-09-25 |
...
| # | OpenSpec id | Principal goal (PRD refs) | Deliverable | Exit criteria | Est. |
|---|---|---|---|---|---|
| **M0** | `bootstrap` | Make the repo itself the product evidence: strict-TS toolchain, CI, and spec-driven governance green from the first commit — and the `mcprelay` name reserved before the crowded space claims it (P6; §11; §13) | Repo scaffold: strict TS, build, vitest, lint/format; **`openspec init`** + `project.md` (PRD = product truth); CI incl. `openspec validate`; ADR-0001 (repo layout); LICENSE; CLI skeleton (`--help`, `version`, exit codes); **npm placeholder publish** (name reservation, §11) | Clean clone → `npm install && npm test` (smoke) green; `openspec validate` clean; `mcprelay --version` runs; npm name reserved | 1 n |
| **M1** | `proxy-stdio` | Become the invisible middleman: any stdio server wrapped 1:1, messages passing through semantically unchanged, every intercepted call visible as one structured log line — demo beat 1 (§1; FR-P1–P6, FR-O1) | Transparent stdio proxy + structured logs: FR-P1–P6, FR-O1; `run` command | Demo step 1 on a real server; FR-P2 fidelity matrix green; logs readable | 2–3 n |
| **M2** | `retry-pipeline` | Classify failures honestly before retrying them: timeout + bounded backoff gated by the D4 taxonomy, so side-effect risk is never guessed — a disciplined retry pipeline, not a hopeful one (§6.3; FR-R1–R2; D4) | Timeout + retry with the D4 failure-class taxonomy (cancellation, `isError`, `input_required`, timeout⇄`idempotent`); config file + CLI flags (FR-C begins) | D4 class table tested; flaky server retried; cancelled calls not retried | 1–2 n |
| **M3** | `dlq-sqlite` | Never lose a failed call: the DLQ exists — durable, redacted capture completes before the client sees the error and survives restart — demo beat 3 (§1; FR-R3, FR-R6; NFR-4/5) | DLQ capture: `QueueProvider` port + SQLite adapter (FR-R3, FR-R6); hash-raw/redact-persist (NFR-4/5); Store port + audit; `replay list/inspect`; `validate` completes FR-C1/C2 | Demo step 3; durability test (NFR-5): kill-after-error leaves the record | 2 n |
| **M4** | `replay-cli` | Ship the differentiator: replay as redrive for side effects — result captured in the audit trail, duplicate effects guarded — demo beat 4 (§1 *What replay means*; FR-R4–R5) — and go public as the first usable release (§9) | Replay: `--dry-run`/`run <id>`, result capture, dedup/`--force` (FR-R4–R5), atomic `resolve()`; **first public release: repo public + real npm publish + honest README** | Demo step 4, ending on the visible side effect; `npx mcprelay` works from npm | 2 n |
| **M5** | `policy-engine` | Guardrails without surprises: declarative allow/deny per tool and per argument, dry-run before enforcement, denials counted but never hidden from `tools/list` — demo beat 2 (§1; FR-Y1–Y6, esp. FR-Y5) | Policy: FR-Y1–Y6; `policy test` | Demo step 2 (block + dry-run); policy suite green | 3 n |
| **M6** | `http-transport` | Extend the same pipeline beyond stdio: 1:1 streamable HTTP toward a remote server with identical policy/DLQ semantics — no stdio-only shortcuts (§6.1; §4 secondary persona) | Streamable HTTP toward remote servers (same pipeline) | Filesystem + one remote server under the same policy/DLQ config | 2 n |
| **M7** | `rabbitmq-batch` | Prove the `QueueProvider` port is real: the same DLQ/replay flow over a real message broker plus batch replay, concurrent-safe — the `docker compose up` demo works (§6.2; FR-R6; D5; §9) | RabbitMQ adapter (D5 topology); batch replay (`run --all --filter`); `docker compose up` demo | Same DLQ/replay flow over `queue.provider: rabbitmq`; concurrent replay safe (atomic `resolve()`); compose demo reproducible | 3–4 n |
| **M8** | `metrics-report` | Answer "what happened?": per-tool latency, error rate, and caller attribution persisted and reportable from the CLI — demo beat 5 (§1; FR-O2–O3; UC4) | FR-O2–O3: per-tool metrics in Store; `report` (table + `--json`); retention (NFR-9). FR-O4 (`/metrics`) = its own post-M8 change if wanted | Demo step 5 with real numbers; metrics match logs | 2–3 n |
| **M9** | `auth-identities` | Give every call a caller: API-key identities feed policy decisions and `report` attribution; tokens validated at the edge, never forwarded upstream (FR-A1, FR-A3; §5.1) | FR-A1, FR-A3 (API keys → identities); FR-A2 (JWT) = post-M9 stretch | Identities feed policy decisions and `report` attribution | 2–3 n |
| **M10** | `release-1-0` | Ship v1.0 to the world: README GIF hook, ≥3 documented examples, launch post — every §9 gating metric green (§9; §1) | README w/ GIF + 3–4 ADRs; `/examples` (≥3 recipes); awesome-list PRs; launch post | All §9 gating metrics green | 1–2 n |
